...
def ofdpex1_finding_sap(G, s, t, iteration, draw=False, pos=None, debug=False):
    ...
    while queue:
        ...
        if debug:
            print "finding", u, "->", v,
        if draw:
            ofdpex1_draw_graph(G, pos, "ofdpex1", paths=True, current=(u,v), parity=rcvd_parity)
        if G.node[v]["type"] == "free":
            ...
            if v == t:
                if G.node[t]["prev"][0][1] != G.node[v]["FHB_prty"] and rcvd_parity:
                    print "continuing"
                    continue
...
def ofdpex1_remove_path(G, s, t, draw=False, pos=None, debug=False):
    queue = deque()
    queue.append((s, G.node[s]["next"][0]))
    del(G.node[s]["next"][0])
    while queue:
        msg = queue.popleft()
        u = msg[0]
        ...
        if v == t:
            ...
            del(G.node[v]["prev"][0])
            queue.append((v, G.node[v]["next"][0]))
            del(G.node[v]["next"][0])
